...
class EditorActivity : AppCompatActivity() {
    ...
    override fun onCreate(s: Bundle?) {
        ...
        b.btnReset.setOnClickListener        { animateBtnPress(b.btnReset)        { resetAll() } }
        ...
        b.previewContainer.viewTreeObserver.addOnGlobalLayoutListener(
            object : ViewTreeObserver.OnGlobalLayoutListener {
                override fun onGlobalLayout() {
                    ...
                    val toolbarH = b.toolbar.height.takeIf { it > 0 }
    ...
    private fun loadData() {
        ...
        // Works with no images — shows dark bg + clock only
        ...
        with(b.editorView) {
            clockX   = WallpaperPrefs.getClkX(this@EditorActivity)
            clockY   = WallpaperPrefs.getClkY(this@EditorActivity)
            clockSz  = WallpaperPrefs.getClkSz(this@EditorActivity)
            clockRot = WallpaperPrefs.getClkRot(this@EditorActivity)
            dateX    = WallpaperPrefs.getDateX(this@EditorActivity)
            dateY    = WallpaperPrefs.getDateY(this@EditorActivity)
            dateSz   = WallpaperPrefs.getDateSz(this@EditorActivity)
            dateRot  = WallpaperPrefs.getDateRot(this@EditorActivity)
            subjX    = WallpaperPrefs.getSubjX(this@EditorActivity)
            subjY    = WallpaperPrefs.getSubjY(this@EditorActivity)
            subjSc   = WallpaperPrefs.getSubjSc(this@EditorActivity)
            subjRot  = WallpaperPrefs.getSubjRot(this@EditorActivity)
            bgRot    = WallpaperPrefs.getBgRot(this@EditorActivity)
            use24hr  = WallpaperPrefs.getUse24(this@EditorActivity)
            showSeconds = WallpaperPrefs.getSecs(this@EditorActivity)
            bgDim    = WallpaperPrefs.getBgDim(this@EditorActivity)
            clockDim = WallpaperPrefs.getClkDim(this@EditorActivity)
    ...
    private fun resetAll() {
        with(b.editorView) {
            ...
            bgRot  = WallpaperPrefs.DEF_BG_ROT
            bgDim  = 0f; clockDim = 0f; subjDim = 0f
        }
        loadedBgBmp?.let { bmp ->
        ...
        b.switch24hr.isChecked = false; b.switchSeconds.isChecked = false
        b.chipPickColor.isChecked = false
        b.colorPicker.animate().alpha(0f).setDuration(150).withEndAction {
    ...
    private fun switchPanel(p: Panel) {
        ...
        out.animate().alpha(0f).translationX(-28f).setDuration(150)
    ...
    private fun showPanel(p: Panel) {
        panel = p
        val inP = panelView(p)
        inP.alpha = 0f; inP.translationX = 28f; inP.visibility = View.VISIBLE
        inP.animate().alpha(1f).translationX(0f).setDuration(220)
    ...
    private fun panelView(p: Panel): View = when (p) {
        Panel.BACKGROUND -> b.panelBg
        Panel.TIME       -> b.panelTime
        Panel.SUBJECT    -> b.panelSubject
    }
    ...
    private fun setupPanels() {
        ...
        // Font status
        val ftF = File(filesDir, WallpaperPrefs.FILE_FONT)
        if (ftF.exists()) b.tvFontStatus.text = ftF.name else b.tvFontStatus.text = "System default"

        // Color pick chip + picker
        ...
        b.chipPickColor.setOnCheckedChangeListener { _, checked ->
            if (checked) {
                ...
                b.colorPicker.animate().alpha(1f).setDuration(220)
                    .setInterpolator(DecelerateInterpolator(1.5f)).start()
            } else {
                b.colorPicker.animate().alpha(0f).setDuration(160).withEndAction {
                    b.colorPicker.visibility = View.GONE
                }.start()
            }
        }
    ...
    private fun updateSwatch(c: Int) {
        ...
        gd.setStroke((1.5f * dp).toInt(), 0x33000000)
    ...
    private fun onFont(uri: Uri) {
        ...
        b.editorView.setFontOnly(tf)
        b.tvFontStatus.text = name
        toast("Font applied")
    }
    ...
    private fun replaceImg(uri: Uri, file: String) {
        try {
            ...
            // Clear stale positions when new image is picked
            ...
            b.previewCard.animate().alpha(0.3f).setDuration(100).withEndAction {
                ...
                b.previewCard.animate().alpha(1f).setDuration(260)
                    .setInterpolator(DecelerateInterpolator()).start()
            }.start()
        } catch (e: Exception) { toast("Error: ${e.message}") }
    }

    // ── Save & launch wallpaper ───────────────────────────────────────────────
    private fun save() {
        with(b.editorView) {
